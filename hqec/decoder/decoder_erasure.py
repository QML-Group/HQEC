...
def calculate_recovery_rate_fixed_weight_multiprocessing(n, k, stabilizers, logical_operators, use_multiprocessing=False):
    ...
    Args:
    n (int): Number of repetitions to generate random fixed weight erasure vectors.
    k (int): Fixed number of erasure errors (1s) in the vector.
    stabilizers (list of str): List of Pauli operator strings representing stabilizers.
    logical_operators (list of str): List of logical operator strings.
    use_multiprocessing (bool): If True, use multiprocessing to speed up calculations.
    ...
    operator_list = stabilizers + logical_operators
    num_logical_operators = len(logical_operators)
    vector_length = len(logical_operators[0])

    # Generate fixed weight erasure vectors for multiprocessing
    erasure_vectors = [generate_fixed_weight_erasure_vector(k, vector_length) for _ in range(n)]
    params = [(operator_list, vec, num_logical_operators) for vec in erasure_vectors]

    if use_multiprocessing:
        with multiprocessing.Pool() as pool:
            results = pool.map(calculate_recovery_rate_single_process, params)
    else:
        results = [calculate_recovery_rate_single_process(param) for param in params]
...
# n = 1000
# k = 3
# stabilizers = ['IXYZ', 'ZZXY', 'YXIZ']
# logical_operators = ['XXXX', 'ZZZZ']
# vector_length = len(stabilizers[0])
# use_multiprocessing = True
# recovery_rate = calculate_recovery_rate_fixed_weight_multiprocessing(n, k, stabilizers, logical_operators, vector_length, use_multiprocessing)
# print("Recovery Rate:", recovery_rate)


def calculate_recovery_rate_fixed_weight_and_complementary(k, stabilizers, logical_operators):
    """
    Calculate the recovery rate from fixed weight erasure errors using multiprocessing.
    ...
    # Generate fixed weight erasure vectors for multiprocessing
    erasure_vector = generate_fixed_weight_erasure_vector(k, vector_length)
    erasure_vector_complementary = generate_complementary_vector(erasure_vector)
    param = (operator_list, erasure_vector, num_logical_operators)
    params_complementary = (operator_list, erasure_vector_complementary, num_logical_operators)

    result = calculate_recovery_rate_single_process(param)
    result_complementary = calculate_recovery_rate_single_process(params_complementary)

    if result == result_complementary:
        print("True")
    else:
        print("False")


def calculate_recovery_rate(n, p, stabilizers, logical_operators):
    """
    Calculate the recovery rate from erasure errors for a given set of stabilizers and logical operators.
    ...
    for _ in range(n):
        erasure_vector = generate_erasure_vector(p, len(operator_list[0]))
        if can_recover_from_erasure(operator_list, erasure_vector, num_logical_operators):
